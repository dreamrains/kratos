...
class TestStatisticalTestRecommendation:
    ...
    def test_compare_periods_keeps_period_rows_descriptive(self, compare_env):
        """compare_periods must not turn ordered period rows into a generic significance test."""
        ...
        assert "independent-group" in parsed["inference_guidance"]["reason"]
        assert "effect magnitude and confidence interval" in parsed[
            "inference_guidance"
...
class TestAutoGeneratedLimitations:
    ...
    def test_method_specific_sample_inadequacy_generates_limitation(self, tmp_path):
        """A failed method-specific adequacy check should generate a limitation."""
        from data_agent import config
        from data_agent.config import AgentConfig
        ...
        try:
            ...
            evidence = json.dumps({
                ...
                "sample_adequacy": {
                    "status": "inadequate",
                    "design": "repeat_purchase_rate",
                    "reason": "too few repeat purchasers for the selected interval method",
                },
            })
            ...
            assert has_sample_warning, f"样本充分性失败应生成局限性提示，实际: {auto_lim}"
        finally:
            config._config = old_cfg
...
class TestPromptConfidenceRules:
    ...
    def test_prompt_distinguishes_descriptive_period_comparison_from_inference(self):
        """System prompt must not manufacture significance for a descriptive comparison."""
        from data_agent.agent.prompts import build_system_prompt
        ...
        assert "compare_periods 默认为描述性" in prompt
        assert "通用显著性检验" in prompt
