...
class TestABTestEdge:
    ...
    def test_real_data_ab(self, env):
        ...
        from data_agent.tools.data_io import load_data
        from data_agent.tools.statistics import ab_test
        load_data(str(reference_data_path("savings_card_before_after")), name="ba")
...
class TestRealDataPipeline:
    ...
    def test_ecard_order_analysis(self, env):
        """省钱卡订单分析流程。"""
        if not reference_data_path("savings_card_orders").exists():
            pytest.skip()
        from data_agent.tools.data_io import load_data
        from data_agent.tools.eda import distribution_analysis
        from data_agent.session.workspace import workspace

        load_data(str(reference_data_path("savings_card_orders")), name="card")
    ...
    def test_large_dataset_performance(self, env):
        """大数据集(13K+ rows)加载性能。"""
        if not reference_data_path("savings_card_user_payments").exists():
            pytest.skip()
        import time
        from data_agent.tools.data_io import load_data
        from data_agent.session.workspace import workspace
        ...
        t0 = time.time()
        load_data(str(reference_data_path("savings_card_user_payments")), name="big")
