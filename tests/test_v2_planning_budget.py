...
def test_budget_counts_exact_system_messages_tools_and_clarifications():
    captured = {}
    ...
    assert estimate.estimated_input_tokens == 321
    assert estimate.available_input_tokens == 900
    assert captured["messages"][0]["role"] == "system"
    assert "clarifications" in captured["messages"][0]["content"]
    assert captured["tools"][0]["type"] == "function"
    assert captured["tools"][0]["function"]["name"] == "submit_analysis_plan"
