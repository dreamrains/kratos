...
- **Material cleaning confirmation**: Destructive or meaning-changing cleaning creates a candidate and requires a resolved confirmation receipt bound to the exact dataset version and transformation proposal before promotion. The legacy `confirmed` Boolean remains a rejected read-only compatibility input and must not authorize a mutation; remove it only after no supported client or persisted queued tool request can emit it. Raw data and prior analysis-copy versions remain available for audit.
- **Analysis assurance contracts**: New executable plans use `analysis_requirement.v1`; new evidence uses server-bound `evidence_record.v2`. Legacy plans and evidence may be normalized only while loading old persisted state, never written as a second authority.
- **Claim and context limits**: Final analysis claims are deterministically audited against current evidence and requirements before publication; optional judging cannot override a blocker. Under context pressure, compaction retains a bounded trust capsule and may reduce breadth, but cannot strengthen an unsupported claim.
