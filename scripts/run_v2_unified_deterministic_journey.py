...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Run or validate the unified-entry owner, incident, and SSE "
            "deterministic evidence journey."
        )
    )
    ...
    if args.evidence is not None:
        ...
        with tempfile.TemporaryDirectory(prefix="data-agent-v2-deterministic-") as temp:
            evidence = collect_unified_deterministic_evidence(
                temp,
                fixture_path=Path(scenario.fixture),
