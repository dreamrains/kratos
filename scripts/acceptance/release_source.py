...
def _git_filtered_blob_ids(root: Path, paths: list[str]) -> dict[str, bytes]:
    """Return current-worktree blob IDs after Git's configured clean filters."""
    ...
    for relative in paths:
        ...
        if batch and (
            len(batch) >= 128
            or batch_argument_chars + argument_chars
            > _HASH_OBJECT_BATCH_ARGUMENT_CHARS
        ):
            flush()
