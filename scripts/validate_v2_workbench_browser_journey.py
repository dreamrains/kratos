...
def main() -> int:
    ...
    parser.add_argument(
        "--journey", choices=("planning", "interaction"), default="planning"
    )
    ...
    validator = (
        validate_provider_neutral_journey
        if args.journey == "planning"
        else validate_provider_neutral_interaction_journey
    )
    result = validator(receipt, expected_source_digest=source.source_digest)
    print(
        json.dumps(
            {
