...
2. **多路线计划**：planner 合同扩展为 routes[]（1-3 条），每条一个 kind+参数；turn 执行按序/并行跑全部路线，findings 汇总
   - 验收：S3（多文件后）= 趋势 + 关联两条路线一个 turn
...
4. **配对检验的口径选择**（sum vs mean 聚合）涉及业务语义，默认 needs_input 一次而非自动选择——与"会话内自动推进"标准（M2-D 确认门标准）的边界需在 B1 设计时明确：口径选择属于"分析语义"非"破坏性操作"，倾向给默认值+可改
