...
- **2026-08-23 增补输入**：`docs/audit/2026-08-23-v1-v2-parity-audit.md`（v1→v2 四层功能对齐审计；本计划 §8 沿用清单据其增补 B2/B3/B4/B7）
...
| 层级 | 机制 | 信任等级 | 状态 |
|---|---|---|---|
| **L0 意图门（非分析意图）** | 分析类 → 进 L1-L4 管线；**知识问答/分析咨询** → 受约束 LLM 直接回答（无数据断言、无 findings、明确标注为咨询）；**闲聊/问候** → 规则层短回应（0 模型成本）；**结果追问** → 携带上一轮 findings 进入规划上下文。v1 的 9 类两层意图体系（agent/llm_intent.py，259 测试全绿）仍在 /legacy 下可用，可参考其分类口径在 v2 重建轻量版 | 咨询类不进 claim 体系；对话历史进入 planner 上下文 | **2026-08-22 补充**，随 B4 产品入口实施（planner 上下文扩展随 B2） |
...
7. **Track1 确定性推荐（沿用 v1 双轨推荐的执行轨）**：新端点基于 `_missing_prerequisites_for_kind` + 列角色画像输出三态推荐（ready：当前数据可直接跑的 kind+绑定建议 / needs_confirmation：缺分析单位或口径待确认 / blocked：缺列/脏列及原因），上传后与 needs_input 卡同呈现，支持点击预填发起
   - 验收：任一 test_doc 文件上传后给出三态清单，且每条 ready 项可一键发起并成功运行；零额外 LLM 成本
8. **period_comparison kind（补 compare_periods 退步项）**：两周期聚合对比（环比/同比口径披露）+ 可比性检查（周期完整性/口径变化），claim ceiling inferential
   - 验收：游戏A内购数据"对比 3 月与 10 月内购收入"给出带 CI 的差异与可比性披露
9. **派生列/通用变换评估**：组合分析单位（B2.3）与多数据集都需要派生列能力——评估以 DatasetRegistry.derive 为底座暴露受约束变换（筛选/聚合/组合列派生），先设计后实施
   - 验收：设计文档 + 若采纳则 S5 重放可用组合列"流量主×广告主"为单位
...
5. **熟练度语言分级（沿用 v1 prompts.py 三级）**：beginner/intermediate/advanced 检测（可用词+历史）→ 叙述生成的 wording_style 输入（plain_language/balanced/technical_concise，搬 v1 synthesis_policy.py:294-300 维度设计）；默认面向非专业用户（项目定位）
   - 验收：同一 S2 结果在 beginner 下无术语裸奔、advanced 下保留统计表述；分级来源披露
6. **替代解释纪律（沿用 v1 假设集理念，2026-08-23 审计结论）**：叙述生成器的必选维度——每个 inferential 结论段必须附"竞争解释"（数据内可见的替代解释+为何当前证据倾向主解释+何种证据能区分）；对应 v1 hypotheses.py 的 primary/alternative/baseline 三段式，但由 B3 按当前 findings 生成而非模板拼接；v1 的文本重叠匹配机制不搬
   - 验收：S2 重放答案含"时间趋势/结构变化等替代解释与区分条件"段；人工维度 alternative_explanations 不再依赖 caveat 兜底
7. **Track2 探索性建议（双轨推荐的探索轨）**：叙述生成器按 findings+数据画像生成 2-4 条探索方向建议，必须声明所需数据/条件与当前不可执行原因（与 L4 遥测联动）
   - 验收：S3 重放给出"补充 banner/激励视频数据可做占比与关联"类带条件的探索建议

### B4 交互层：产品入口（Workbench → 真正的网页入口）

**对齐基线（2026-08-23 审计）**：`docs/audit/2026-08-23-v1-v2-parity-audit.md` §四 的 v1 web 十类功能清单即 B4 移植清单（会话管理含回退重发、任务看板、图表内联/补充区/导出、斜杠命令、KaTeX/token 环、知识面板随 B7）；v2 独有交互（授权门/估算/规划确认/needs_input/steer/结构化块）全部保留。
...
- B1-B5+B7 验收 + 稳定观察后，审计并删除旧 AgentLoop 入口与代码；与首次切换解耦（既有规则）。删除前必须确认 B7 已承接知识/记忆/证据资产（否则数据迁移先行）

### B7 知识/记忆/证据系统接入（2026-08-23 审计新增）

v1 的 KnowledgeLibrary + MemoryStore（候选→确认）+ EvidenceStore + 检索注入（每轮 compose 进系统提示，知识/记忆冲突检测）+ 管理面板，在 v2 中整层缺失。这是跨会话资产层，与 v2 会话内 findings/commitments 是两类东西，需要设计先行：

1. **设计评估切片**（先出设计文档，不写码）：v2 的 finding/claim 体系与 EvidenceRecord 的映射；检索注入点在哪层（L0 咨询层？B3 叙述生成器输入？planner 上下文？）；记忆候选的提取与确认流程在 v2 事件流上的挂接；管理面板与 B4 的关系
2. **实施切片**：按设计结论实施（依赖 B3/B4 的形态）
- 验收：跨会话场景——会话 A 确认一条业务口径记忆，会话 B 的分析叙述正确引用且冲突时披露

## 5. 批次顺序与依赖

B1（引擎）→ B3（表达）为关键路径：B1 产出更丰富的 findings，B3 才有可叙述的内容；B2 与 B3 可并行（不同层）；B4 依赖 B2 的多文件与 B3 的卡片化内容；B5 每批滚动更新；B7 设计评估可与 B3 并行启动（纯文档），实施依赖 B3/B4；B6 最后。
建议节奏：B1 → B3 → B2 → B4 → B7 实施 → B6，每批一个 source 冻结 + 成组真实验证。
**引擎扩容候选队列**（遥测驱动，按 L4 报告频率插入 B1 类批次）：funnel_analysis / cohort 矩阵 / contribute_decomposition / DID / segmentation / what_if / forecast 模型族增强——均为 v1 已有、v2 待建（审计 §一 📋 项）。
