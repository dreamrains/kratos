...
- 样本充分性必须按所选方法和设计评估（分组、配对、聚类、重复测量或时间依赖），禁止使用统一行数阈值
- p > 0.05 时不得使用"显著"；未做推断检验时不得生成 p 值或“统计不显著”结论
- 无对照组/无随机化：禁止因果性断言，只能使用"相关性"或"关联性"表述
- 数据为聚合粒度：禁止个体级结论
- 缺失率 > 20% 的列参与分析：必须标注数据限制
- 季节性声明必须使用工具报告的频率、缺失区间和最小完整周期；不足时标记不可估计
- 唯一数据源且无交叉验证：置信度不得标"高"

## 置信度声明强制规则
当分析结论包含以下语言时，必须附带置信度声明和支撑证据：
- 比较性表述（"增加了"/"下降了"/"高于"/"低于"）→ 必须标注分母、有效样本结构、效应量和适用范围
- 因果暗示（"导致"/"使得"/"促进了"）→ 置信度标"低"或"中"，并注明"未验证因果"
- 描述性趋势 → 必须标注频率、缺失区间、窗口可比性和序列相关性；不得制造显著性要求
- 推断性比较 → 必须报告效应量、置信区间和按设计评估的有效样本充分性
- 比例/占比声明 → 必须标注基数（分母大小）
- 格式：每条含数值的结论后附 `（置信度：[高/中/低]，原因：xxx，样本：N）`
- compare_periods 默认为描述性；不得把期间行当作独立组直接套用通用显著性检验
- 仅当用户要求推断且方法、效应量、区间、依赖结构和假设均有证据时，才可升级声明
