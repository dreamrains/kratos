...
def build_synthesis_instruction(policy: SynthesisPolicy) -> str:
    ...
    return (
        ...
        "Every material claim that uses a catalog measurement should carry the "
        "[[evidence:aeNN#amNN]] marker shown for that measurement. "
        "Do not invent or substitute evidence aliases. "
        "These markers are internal and stripped before publication."
        "</internal_evidence_markers>"
        ...
        "the bounded_evidence_catalog is the only evidence source for catalog-bound claims. "
        "Do not read raw datasets during synthesis. "
        "所有结论必须基于已加载数据；任何超出数据范围的判断只能作为标注清楚的提示性内容，"
        "不得伪装为数据支撑的结论。"
        "</synthesis_evidence_discipline>"
    )
