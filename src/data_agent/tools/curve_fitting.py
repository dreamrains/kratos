...
@registry.register(
    name="curve_fitting",
    description="比较幂律、指数和对数曲线族；仅描述观测区间，不生成外推或因果结论。",
    schema_overrides={
...
def curve_fitting(name: str, series_columns: str = "", x_col: str = "", y_col: str = "", zero_values: str = "exclude") -> ToolResult:
    ...
    if series_columns:
        ...
        mode = "long_columns"
    receipt = method_receipt(name, method="curve_family_comparison", status="limited", effective_n=len(points), parameters={"mode": mode, "zero_values": zero_values}, limitations=["拟合只描述当前观测范围，不支持外推或因果解释。"], claim_ceiling="descriptive")
    if len(points) < 5:
